	...
	public static Map<String, Object> convertSetToMap(Set<Attribute> attributes) {
		...
		if (attributes != null && !attributes.isEmpty()) {
			...
			for (Attribute attribute : attributes) {
				 ...
				 * addValueToMap(row, value, attribute); String enableValue =
				 * row.get("__ENABLE__").toString(); if (enableValue.equalsIgnoreCase("true")) {
				 * row.put("status", "active"); } else { row.put("status", "inactive"); }
